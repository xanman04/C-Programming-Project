...
A networked, multi-player Blackjack game implemented in C using TCP sockets.
...
Overview
This project implements a simple but fully functional Blackjack game where a central server manages the deck, deals cards, coordinates turns, and determines results. Up to five clients can connect over TCP and play against the dealer in real time.
Everything is written in standard C with no external libraries. The project demonstrates systems programming concepts such as socket communication, select-based connection handling, modular game logic, and structured data design.
Features
...
✔ Clear Client UI
Clients receive simple text commands from the server and display them as a playable interface:
Dealer’s up card
Player’s hand
Hit/Stand prompts
Result messages
...
README.md
Makefile / build instructions (your environment)
How the Game Works
1. Server
